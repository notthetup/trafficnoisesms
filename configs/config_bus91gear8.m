...
%% Audio
%audioFile = 'buss_91_gear8';
%audioFile = 'buss_30_gear3';
audioFile = 'iv_50_vx4_6s-destoned';
Fs = 44100;
...
nPeaks = 20;              % Max peaks captured per frame
pHThresDB = -65;          % Peak Height Treshold - Only peaks above this value are considered
pHThres = 10^(pHThresDB/20);
pWThresh = 20;             % Peak Width Treshold - The minimum width between peaks [Hz]

guideIncFactor = 0.5;    % Additional factor of total guides each pass.
minGuideEnergy = 0.008;

pdp.maxFreqDiff = 2;       % Max diff in peaks to be considered in same guides
pdp.nPeaks = nPeaks;
pdp.pHThres = pHThres;
pdp.pWThresh = pWThresh;
pdp.alpha = 0.25;            % Trajectory vs next peak ratio
pdp.pFreqMin = 20;
pdp.pFreqMax = 15000;
pdp.cBandsNormFac = 20;     % Normalization factor critical bands BW.
...
cF.numPass = length(stftParams)-1;
...
cF.guideIncFactor = guideIncFactor*maxGuides;
...
cF.rAudioNoisefName = sprintf('./synths/%s/%s_%s_%s_noise.wav',cAudioFileName,currDateTime,cAudioFileName,'reSynth');
